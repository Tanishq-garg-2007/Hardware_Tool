...
def parse_bootlog(filepath: str) -> BootlogInfo:
    ...
    # Dynamic Extraction for AES (e.g. AES-128, AES-256)
    aes_matches = set(re.findall(r"\baes(?:-?128|-?192|-?256)?\b", content, re.IGNORECASE))
    if aes_matches:
        for match in aes_matches:
            match_upper = match.upper()
            if match_upper == "AES128": match_upper = "AES-128"
            elif match_upper == "AES192": match_upper = "AES-192"
            elif match_upper == "AES256": match_upper = "AES-256"
            info.crypto_algos.add(match_upper)

    # Dynamic Extraction for RSA (e.g. RSA-2048, RSA-4096)
    rsa_matches = set(re.findall(r"\brsa(?:-?\d{3,4})?\b", content, re.IGNORECASE))
    if rsa_matches:
        for match in rsa_matches:
            match_upper = match.upper()
            if re.match(r"^RSA\d+$", match_upper):
                match_upper = match_upper.replace("RSA", "RSA-")
            info.crypto_algos.add(match_upper)
...
def _load_cve_database(cve_db_path: str = None) -> list:
    """
    Loads relevant firmware/bootloader/kernel CVEs directly from the local NVD SQLite database.
    If the database is not found, attempts legacy folder crawl or triggers update.
    """
    try:
        from cve_updater import get_nvd_db_path, query_firmware_cves
        ...
        if db_file.exists():
            ...
            entries = query_firmware_cves(all_targets)
            if entries:
                log.info(f"Loaded {len(entries)} relevant firmware CVE entries from NVD SQLite database ({db_file})")
                return entries
    except Exception as e:
        log.warning(f"Could not load from NVD SQLite database: {e}")

    # Fallback to file directory if provided
    if cve_db_path and Path(cve_db_path).exists() and Path(cve_db_path).is_dir():
        path = Path(cve_db_path)
        log.info(f"Crawling CVE directory: {path}...")
        entries = []
        for root, _, files in os.walk(path):
            for file in files:
                if file.startswith("CVE-") and file.endswith(".json"):
    ...
    log.warning("No CVE database entries found. Run database update to populate NVD database.")
    return []
...
def _sync_get_id(cve_obj: dict) -> str:
    return cve_obj.get("id") or cve_obj.get("cve_id") or cve_obj.get("cveMetadata", {}).get("cveId", "")

def _sync_get_last_modified(cve_obj: dict) -> str:
    return cve_obj.get("last_modified") or cve_obj.get("lastModified") or cve_obj.get("cveMetadata", {}).get("dateUpdated", "")

def _sync_get_published(cve_obj: dict) -> str:
    meta = cve_obj.get("cveMetadata", {})
    return cve_obj.get("published") or meta.get("datePublished") or meta.get("dateUpdated") or meta.get("dateReserved", "")

def _get_description(cve_obj: dict) -> str:
    if "description" in cve_obj and isinstance(cve_obj["description"], str) and cve_obj["description"]:
        return cve_obj["description"]
...
def _get_severity_and_score(cve_obj: dict) -> tuple:
    # 1. Direct fields from NVD SQLite ingestion
    if "base_score" in cve_obj and cve_obj["base_score"] is not None:
        try:
            score = float(cve_obj["base_score"])
            if score > 0.0:
                sev = cve_obj.get("severity") or ("HIGH" if score >= 7.0 else ("MEDIUM" if score >= 4.0 else "LOW"))
                return str(sev).upper(), score
        except (ValueError, TypeError):
            pass

    # 2. Check standard NVD metrics dictionary
    if "metrics" in cve_obj:
        ...
        for key in ("cvssMetricV31", "cvssMetricV30", "cvssMetricV40"):
            if key in metrics and metrics[key]:
                ...
                return str(sev).upper(), float(score) if score is not None else 0.0
    ...
    # 3. Check MITRE v5 containers fallback
    containers = cve_obj.get("containers", {})
    metrics = []
    if "cna" in containers:
        metrics.extend(containers["cna"].get("metrics", []))
    if "adp" in containers and isinstance(containers["adp"], list):
        for adp in containers["adp"]:
            metrics.extend(adp.get("metrics", []))

    for m in metrics:
        if "cvssV3_1" in m:
            score = m["cvssV3_1"].get("baseScore", 0.0)
            sev = m["cvssV3_1"].get("baseSeverity", "UNKNOWN")
            return str(sev).upper(), float(score) if score is not None else 0.0
        elif "cvssV3_0" in m:
            score = m["cvssV3_0"].get("baseScore", 0.0)
            sev = m["cvssV3_0"].get("baseSeverity", "UNKNOWN")
            return str(sev).upper(), float(score) if score is not None else 0.0
        elif "cvssV4_0" in m:
            score = m["cvssV4_0"].get("baseScore", 0.0)
            sev = m["cvssV4_0"].get("baseSeverity", "UNKNOWN")
            return str(sev).upper(), float(score) if score is not None else 0.0
        elif "cvssV2_0" in m:
...
def _get_references(cve_obj: dict) -> list:
    if "references" in cve_obj and isinstance(cve_obj["references"], list):
        refs = []
        for r in cve_obj["references"]:
            if isinstance(r, str): refs.append(r)
            elif isinstance(r, dict) and r.get("url"): refs.append(r["url"])
        return refs
    cna = cve_obj.get("containers", {}).get("cna", {})
    return [r.get("url", "") for r in cna.get("references", []) if r.get("url")]

def _get_weaknesses(cve_obj: dict) -> list:
    if "weaknesses" in cve_obj and isinstance(cve_obj["weaknesses"], list):
        return [str(w) for w in cve_obj["weaknesses"]]
    cna = cve_obj.get("containers", {}).get("cna", {})
    cwes = []
...
def _get_cpe_matches(cve_obj: dict) -> list:
    # 1. Direct from SQLite structured rows
    if "cpe_matches" in cve_obj and isinstance(cve_obj["cpe_matches"], list) and cve_obj["cpe_matches"]:
        return cve_obj["cpe_matches"]

    # 2. Extract from NVD configurations node
            ...
            for node in cfg.get("nodes", []):
                for cm in node.get("cpeMatch", []):
                    ...
                    results.append({
                        ...
                        "config_operator": cfg.get("operator", "OR")
                    })
        if results:
            return results

    # 3. Fallback to MITRE containers
    results = []
    cna = cve_obj.get("containers", {}).get("cna", {})
...
def _check_upstream_downstream_date(build_date_str: str, cve_published_str: str) -> dict:
    ...
    try:
        build_dt = datetime.datetime.strptime(build_date_str[:10], "%Y-%m-%d")
        cve_dt   = datetime.datetime.strptime(cve_published_str[:10], "%Y-%m-%d")
        if build_dt > cve_dt:
...
def main():
    ...
    if should_update():
        if args.dry_run:
            log.info("DRY RUN: Database needs update, but skipping download.")

    else:
        log.info("Database is up-to-date (less than 30 days old).")
...
if __name__ == "__main__":
    main()
def find_cve(text: str, db_dir: str = "db_temp") -> str:
    import tempfile
    try:
        if should_update():
            update()
