...
def toggle_target_power(state: int, power_thread=None):
    """
    Safely toggles target power state (0 = OFF, 1 = ON).
    Directly drives RELAY_POWER_PIN (LOW = OFF, HIGH = ON) matching
    capture_uart_boot.py and check_uart_console.py.
    """
    try:
        from hardware_config import get_gpio
        gpio = get_gpio()
        if gpio is not None:
...
# ---------------- MAIN FUNCTION ---------------- #
def run_baud_detection(sampling_time, power_cycle_delay, power_thread):
...
if __name__ == "__main__":
    import sys
    s_time = int(sys.argv[1]) if len(sys.argv) > 1 else 5
    p_delay = int(sys.argv[2]) if len(sys.argv) > 2 else 3
    print(f"[CLI] Testing baud detection with sampling_time={s_time}s, power_delay={p_delay}s")
    result = run_baud_detection(s_time, p_delay, None)
    print(f"[CLI] Detected best baud: {result}")
