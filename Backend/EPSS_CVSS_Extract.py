...
def get_cvss_v2(cve_id: str) -> Dict[str, Any]:
    # 1. Primary: query complete NVD offline database
    try:
        from cve_updater import get_cvss
        res = get_cvss(cve_id)
        if res.get("Base_Score") is not None:
            return res
    except Exception:
        pass

    # 2. Secondary: fallback to CVE_INDEX if present
    v = CVE_INDEX.get(cve_id)
    if not v:
        return {"CVE_ID": cve_id, "Error": None, "Base_Score": "N/A", "Severity": "UNRATED"}
